...
public class MedicationScheduleService {
    ...
    // 특정 날짜의 복약 일정 조회
    public List<MedicationSchedule> getSchedulesByUserAndDate(String userId, LocalDate date) {
        return scheduleRepository.findByUserIdAndTmDate(userId, date);
    }

    // 복약 일정 수정 (날짜, 시간, 복약 여부 업데이트)
    public MedicationSchedule updateSchedule(Long tmIdx, MedicationScheduleUpdateDto dto) {
        Optional<MedicationSchedule> optionalSchedule = scheduleRepository.findById(tmIdx);
